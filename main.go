...
func calculateTask1(input string, nString string) string {
	...
	output := fmt.Sprintf("Частота відмов одноколової системи: %.3f рік^-1\n", omegaSum) +
		fmt.Sprintf("Середня тривалість відновлення: %.3f год\n", tRecovery) +
		fmt.Sprintf("Коефіцієнт аварійного простою: %.5f\n", kAP) +
		fmt.Sprintf("Коефіцієнт планового простою: %.5f\n", kPP) +
		fmt.Sprintf("Імовірність подвійного відмовии: %.5f рік^-1\n", omegaDK) +
		fmt.Sprintf("Фінальна частота відмов із перемиканням: %.5f рік^-1\n", omegaDKS)

	return output
}

func calculateTask2(zPerA, zPerP, omega, t, Pm, Tm, kp float64) string {
	MWA := omega * t * Pm * Tm
	MWP := kp * Pm * Tm
	M := zPerA*MWA + zPerP*MWP

	output := fmt.Sprintf(
		"Математичне сподівання аварійного недовідпущення: %.2f кВт * год\n"+
			"Математичне сподівання планового недовідпущення: %.2f кВт * год\n"+
			"Математичне сподівання збитків від перервання електропостачання: %.2f грн\n",
		MWA, MWP, M,
	)
	return output
}
